...
| 标签管理 | 增 / 改名 / 删除 / **合并到**；点击条目**展开**看该标签下的笔记；展示形态可循环切换 |
| 分组管理 | 增 / 改名 / 删除（可选择「保留笔记」或「一起删除」）/ 上移下移排序；点击条目**展开**看组内笔记 |
...
| **真机展开交互（LG G8 / v1.2.0）** | 分组页、标签页均「点条目展开 → 显示组内 / 带该标签的笔记」；展开区点笔记直接进「查看」预览；两页的列表 ↔ 瀑布流切换都生效（瀑布流下条目为半屏 tile、展开区通栏显示） |
...
| v1.2.0 | 「分组」「标签」页条目支持**点击展开**查看组内 / 带该标签的笔记；两页复用笔记页的卡片样式与列表 ↔ 瀑布流循环切换（瀑布流下展开区通栏）；卡片组件抽成共享 `ui/components/NoteCards.kt`，展示形态枚举移到 `ui/NoteLayout.kt` |
